/*
Fuse

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fuse

import (
	"encoding/json"
	"fmt"
)

// checks if the FinancialConnectionsOwnerNamesInner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FinancialConnectionsOwnerNamesInner{}

// FinancialConnectionsOwnerNamesInner struct for FinancialConnectionsOwnerNamesInner
type FinancialConnectionsOwnerNamesInner struct {
	// Name of the owner
	Data string `json:"data"`
	// Type of name. Possible values are \"name\" or \"alias\"
	Type *string `json:"type,omitempty"`
}

type _FinancialConnectionsOwnerNamesInner FinancialConnectionsOwnerNamesInner

// NewFinancialConnectionsOwnerNamesInner instantiates a new FinancialConnectionsOwnerNamesInner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFinancialConnectionsOwnerNamesInner(data string) *FinancialConnectionsOwnerNamesInner {
	this := FinancialConnectionsOwnerNamesInner{}
	this.Data = data
	return &this
}

// NewFinancialConnectionsOwnerNamesInnerWithDefaults instantiates a new FinancialConnectionsOwnerNamesInner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFinancialConnectionsOwnerNamesInnerWithDefaults() *FinancialConnectionsOwnerNamesInner {
	this := FinancialConnectionsOwnerNamesInner{}
	return &this
}

// GetData returns the Data field value
func (o *FinancialConnectionsOwnerNamesInner) GetData() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *FinancialConnectionsOwnerNamesInner) GetDataOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Data, true
}

// SetData sets field value
func (o *FinancialConnectionsOwnerNamesInner) SetData(v string) {
	o.Data = v
}

// GetType returns the Type field value if set, zero value otherwise.
func (o *FinancialConnectionsOwnerNamesInner) GetType() string {
	if o == nil || IsNil(o.Type) {
		var ret string
		return ret
	}
	return *o.Type
}

// GetTypeOk returns a tuple with the Type field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FinancialConnectionsOwnerNamesInner) GetTypeOk() (*string, bool) {
	if o == nil || IsNil(o.Type) {
		return nil, false
	}
	return o.Type, true
}

// HasType returns a boolean if a field has been set.
func (o *FinancialConnectionsOwnerNamesInner) HasType() bool {
	if o != nil && !IsNil(o.Type) {
		return true
	}

	return false
}

// SetType gets a reference to the given string and assigns it to the Type field.
func (o *FinancialConnectionsOwnerNamesInner) SetType(v string) {
	o.Type = &v
}

func (o FinancialConnectionsOwnerNamesInner) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o FinancialConnectionsOwnerNamesInner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	if !IsNil(o.Type) {
		toSerialize["type"] = o.Type
	}
	return toSerialize, nil
}

func (o *FinancialConnectionsOwnerNamesInner) UnmarshalJSON(bytes []byte) (err error) {
    // This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"data",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(bytes, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varFinancialConnectionsOwnerNamesInner := _FinancialConnectionsOwnerNamesInner{}

	err = json.Unmarshal(bytes, &varFinancialConnectionsOwnerNamesInner)

	if err != nil {
		return err
	}

	*o = FinancialConnectionsOwnerNamesInner(varFinancialConnectionsOwnerNamesInner)

	return err
}

type NullableFinancialConnectionsOwnerNamesInner struct {
	value *FinancialConnectionsOwnerNamesInner
	isSet bool
}

func (v NullableFinancialConnectionsOwnerNamesInner) Get() *FinancialConnectionsOwnerNamesInner {
	return v.value
}

func (v *NullableFinancialConnectionsOwnerNamesInner) Set(val *FinancialConnectionsOwnerNamesInner) {
	v.value = val
	v.isSet = true
}

func (v NullableFinancialConnectionsOwnerNamesInner) IsSet() bool {
	return v.isSet
}

func (v *NullableFinancialConnectionsOwnerNamesInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFinancialConnectionsOwnerNamesInner(val *FinancialConnectionsOwnerNamesInner) *NullableFinancialConnectionsOwnerNamesInner {
	return &NullableFinancialConnectionsOwnerNamesInner{value: val, isSet: true}
}

func (v NullableFinancialConnectionsOwnerNamesInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFinancialConnectionsOwnerNamesInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


